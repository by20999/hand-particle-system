...
- 3D 粒子模型：粒子组成带有厚度的 3D 爱心，也可以切换成全方位 3D 花朵、土星、待爆烟花和自定义文字，模型区预留了“更多”入口。
...
- 自定义文字粒子：输入名字或短句后，粒子可以组成更大、更细腻的文字；不同字体会使用字距、描边、倾斜、波浪和单字旋转等差异化采样。
- 主题配色选择：通过下拉菜单选择主题，支持霓虹、金暖、冰蓝、紫绿、黑金、玫红、激光、落日、电紫等配色与场景氛围变量。
- 背景氛围：支持星云、柔光舞台、极简黑场、烟花夜空等多层电影感背景模式，并加入动态程序化天幕。
- 音乐响应：支持麦克风或本地音频文件，平缓段尽量保持模型形状，鼓点/重音段会触发心跳缩放、快速位移、自身旋转、朝向变化、战斗机式粒子拖尾和 Bloom 律动。
...
10. 切换到“烟花”模型后，张开手掌或点击画面可以触发烟花从上升轨迹爆开。
11. 点击“麦克风”或选择本地音频文件后，粒子会叠加音乐响应，底部会显示主题联动的音频条；点击“停止”可关闭音乐响应。
...
本地音频播放音量已做压低处理，底部音频条会跟随当前主题显示；频谱响应做了降饱和和中心强化处理，低频律动会集中在中间，避免整首歌长时间顶满。

### 模型细节

- 花朵：使用花茎、叶片、萼片、随机混合的外侧护瓣、多层杯状花瓣和中心花芯生成，默认使用红色花瓣和绿色茎叶，花冠缩小并增加粒子密度以强化包裹感。
- 烟花：初始状态为多组上升后的待爆小光团和更明亮的细尾迹；张开手掌后按烟花专属阈值绽放，每朵烟花会保持更清晰的圆润花冠和分组轨迹。
...
- `src/shapes.js`：爱心、花朵、土星、烟花和文字粒子模型的目标点生成。
