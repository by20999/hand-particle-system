...
- 3D 粒子模型：粒子组成带有厚度的 3D 爱心，也可以切换成全方位 3D 花朵、土星、待爆烟花、钻戒、生日蛋糕、气球、自定义文字、姿态视频粒子骨架、图片/Logo 点云和 GLB 模型表面点云。
...
- GLB 模型粒子：上传 `.glb` 后会按三角面面积在模型表面采样，生成真正有深度的 3D 粒子点云；带顶点色的模型会尽量保留顶点颜色，并可单独调节密度、大小、厚度和表面雾化感。
- 姿态视频粒子：上传人物/舞蹈视频后，浏览器本地加载 MediaPipe Pose，识别头、肩、肘、腕、胯、膝、踝等人体关键点，并实时生成发光粒子骨架、关节光点和四肢端点残影；不要求导入 3D 人物模型，可调节骨架密度、跟随频率和光晕。
...
10. 上传图片/Logo 会生成图片粒子；上传 `.glb` 会生成模型表面粒子；上传人物或舞蹈视频会生成跟随视频动作的姿态粒子骨架。
...
- 导入：图片/Logo、GLB 模型与姿态视频导入、加载进度条、失败提示，以及图片/Logo、GLB、姿态视频各自的细节折叠参数。
...
- 姿态：上传人物/舞蹈视频后，系统会用 MediaPipe Pose 提取人体关键点，把肩、肘、腕、胯、膝、踝等连接成发光粒子骨架；骨架会保留左右侧配色、关节光点和四肢端点残影，适合把跳舞视频变成舞台感粒子人体。视频只在本地浏览器中解码和识别，不上传服务器；导入面板可调骨架密度、跟随频率和关节/端点光晕。
...
- GLB 模型：通过 `GLTFLoader` 读取 `.glb`，按网格三角面面积在模型表面高密度采样并归一化到粒子空间；如果网格带有 vertex color，会按三角形重心插值颜色。导入面板可调 GLB 点云密度、大小、前后厚度和表面微抖动，密度偏低会更流畅，密度偏高会更细。
...
图片/Logo 导入会根据画质档位生成更大的采样池：移动端约为粒子数 `2.8` 倍，桌面端约为 `3.4` 倍，`ultra` 最高约 `2.4M` 个候选采样点；GLB 表面采样上限最高约 `980k` 点。姿态视频会按画质档位自适应抽帧识别人体姿态，每帧生成约 `6.2k` 到 `22k` 个骨架采样点，并且只重写当前可见的姿态粒子目标；桌面端会比移动端更高频更新，避免视频姿态驱动把主线程拖慢。
