...
- 演出巡演与编排器：可以选择自动巡演、玫瑰告白、夜场烟花、图形展台、星河心动现场等预设，也可以进入“自定义编排”编辑时间线，让主题、背景、模型、文字、亮度、图片大小、镜头、爆发和定格按片段自动切换；支持从 JSON 文件或 `window.handParticleShows.importPreset()` 导入演出。
- 分层控制面板：演出与视觉、模型、导入、交互、音乐和系统设置分区折叠；常见细项提供 `?` 说明，点击后会显示当前调整项的作用。
...
- 模型：模型缩略图、文字输入、字体预设、模型亮度和“更多”模型预留入口。
- 导入：图片/Logo 与 GLB 模型导入、加载进度条、失败提示和图片/Logo 亮度、大小、轮廓、透明度等细节参数。
- 交互：手势开关、静止按钮、粒子颜色和手势灵敏度。
- 音乐：麦克风、本地音频、停止按钮和音量百分比。
- 系统：自动画质档位、近似 FPS、粒子数量和面板缩放。
...
- GLB 模型：通过 `GLTFLoader` 读取 `.glb`，按网格三角面面积在模型表面高密度采样并归一化到粒子空间；如果网格带有 vertex color，会按三角形重心插值颜色。
...
图片/Logo 导入会根据画质档位生成更大的采样池：移动端约为粒子数 `2.8` 倍，桌面端约为 `3.4` 倍，`ultra` 最高约 `2.4M` 个候选采样点；GLB 表面采样上限最高约 `980k` 点。
